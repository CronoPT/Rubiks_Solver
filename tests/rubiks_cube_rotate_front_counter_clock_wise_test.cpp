...
class rubiks_cube_rotate_front_counter_clock_wise_test: public testing::Test
{   
    protected:
...
TEST_F(rubiks_cube_rotate_front_counter_clock_wise_test, two_sided)
{
    _aux = {WHITE, GREEN, 
            RED  , WHITE};
    ...
    _cube.rotate_face_counter_clock_wise(FRONT, 1);

    ASSERT_EQ(_resulting_cube, _cube);
}

TEST_F(rubiks_cube_rotate_front_counter_clock_wise_test, three_sided)
{
    ...
    _aux = {YELLOW, WHITE, RED,
            GREEN , BLUE, GREEN,
            RED   , GREEN, YELLOW};
    _right = cube::face(3, _aux);

    _aux = {GREEN , YELLOW, ORANGE,
            ORANGE, GREEN , RED,
            RED   , WHITE , ORANGE};
    _left = cube::face(3, _aux);
