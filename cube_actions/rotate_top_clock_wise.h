...
#ifndef __ROTATE_TOP_CLOCK_WISE_H__
#define __ROTATE_TOP_CLOCK_WISE_H__
...
class rotate_top_clock_wise: public search::action<rubiks_cube>
{
    private:
        int _squares;

    public:
        rotate_top_clock_wise(int squares): search::action<rubiks_cube>()
        {   
            _squares = squares;
        }
        ...
        {
            cube.rotate_face_clock_wise(TOP, _squares);
            return cube;
        }
        ...
        friend std::ostream& operator<<(std::ostream& os, const rotate_top_clock_wise& r)
        {
            r.dump_to(os);
            return os;
        }
