...
class rotate_front_counter_clock_wise: public search::action<rubiks_cube>
{
    private:
        int _squares;

    public:
        rotate_front_counter_clock_wise(int squares): search::action<rubiks_cube>()
        {   
            _squares = squares;
        }
        ...
        friend std::ostream& operator<<(std::ostream& os, const rotate_front_counter_clock_wise& r)
        {
            r.dump_to(os);
            return os;
        }
